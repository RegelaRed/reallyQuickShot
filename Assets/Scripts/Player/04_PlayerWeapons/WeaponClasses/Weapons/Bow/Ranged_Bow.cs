...
public class Ranged_Bow : WeaponsBase
{
    ...
    /// <summary>Wrapper for _ammoCounts[Index]</summary>
    private int CurrentAmmoCount { get => _ammoCounts[_currentAmmoIndex]; set => _ammoCounts[_currentAmmoIndex] = value; }

    #endregion
    #region  Tick Weapon

    public override void TickWeapon(WeaponContext weaponContext)
    {
        // Handle reload 
        if (IsReloading)
        {
        ...
        // Handle release
        else if (weaponContext.AttackReleased && _currentCharge > 0f)
        {
            if (!CanAttack)
            {
